...
export default function UnassignedPage({
  ...
  return (
    <>
      <Head>
        <title>TechTool App - Unassigned</title>
      ...
      />
      <Script src="/js/app.js?v=7" strategy="afterInteractive" />
    </>
  );
}
...
export const getServerSideProps: GetServerSideProps<AppPageProps> = async (
  ...
  const replacements: Record<string, string> = {
    ...
    "{{SCRIPT_URL}}": escapeHtml("/unassigned"),
    "{{TICKET_NUMBER}}": escapeHtml(""),
    "{{INITIAL_VIEW}}": escapeHtml("unassigned"),
