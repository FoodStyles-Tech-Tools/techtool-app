...
type DiscordNotifyResponse = { success: boolean } | ApiError;

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<DiscordNotifyResponse>
) {
  ...
  const appUrlBase = process.env.NODE_ENV === "production"
    ? "https://techtool-app.vercel.app/my-tickets?ticket=HRB-"
    : "http://localhost:3000/my-tickets?ticket=HRB-";
  ...
  try {
    ...
    handleApiError(res, error, "Failed to send Discord notification");
  }
}
