...
function orderPriorities(priorities: string[]): string[] {
  const preferred = PRIORITY_ORDER.filter((preferredKey) =>
    ...
    priorities.find((priority) => normalizeSeriesKey(priority) === preferredKey)!
  )
          ...
          >
            ...
            {types.map((type, index) => (
              <Bar
                ...
                fill={getVolumeTypeColor(type, index)}
...
export function ReportResponseTimeChart({ data }: { data: AvgResponseTimeByWeekItem[] }) {
  const priorities = orderPriorities(
    Array.from(new Set(data.flatMap((week) => week.byPriority.map((priority) => priority.priority))))
  )
          ...
          >
            ...
            {priorities.map((priority, index) => (
              <Bar
                ...
                fill={getPriorityColor(priority, index)}
                barSize={WEEK_BAR_SIZE}
                radius={[4, 4, 0, 0]}
                label={{
                  position: "top",
